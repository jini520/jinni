...
`.claude/rules/workflow.md`의 **코드 수정 절대 승인 원칙**과 **구현 승인 게이트**를 따른다:

- **사용자의 명시적 승인 없이는 어떤 코드도 절대 수정하지 않는다.** 시안 선택·방향 동의("~로 하자", "B안이 좋아")는 수정 승인이 아니다 — 내용 확정 후 "코드에 적용할까요?"를 묻고 승인을 기다린다.
- 비자명한 작업(3단계 이상 또는 아키텍처 결정)은 ① 코드베이스 파악 → ② 접근법 2–3가지와 트레이드오프 제시 → ③ **"구현해/진행해/GO" 등 명시적 승인 대기** → ④ 승인 후 구현.
- 즉시 실행 가능(코드 수정 제외): 사용자가 "바로 구현해"라고 명시한 경우.
...
**운영 API 검증**: 반드시 공개 도메인으로 호출할 것 — `https://jejinni.site/api/skills` 형태. `http://116.122.92.238:3000/...`(IP:포트) 호출 금지 — 개발 Mac이 운영 서버와 같은 NAT 뒤에 있어 헤어핀으로 닿을 뿐, 외부 접근 가능 여부를 검증하지 못한다(방화벽은 22번 외 닫힘).
